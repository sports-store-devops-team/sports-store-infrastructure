mock_provider "aws" {
  mock_data "aws_caller_identity" {
    defaults = {
      account_id = "796231892654"
      arn        = "arn:aws:iam::796231892654:user/Niv-Bar"
  ...
  mock_data "aws_iam_session_context" {
    defaults = {
      issuer_arn = "arn:aws:iam::796231892654:user/Niv-Bar"
    }
  }
...
run "expected_account_configuration" {
  ...
  assert {
    condition     = local.account_id == "796231892654" && local.application_image_registry == "796231892654.dkr.ecr.eu-central-1.amazonaws.com"
    error_message = "The ECR registry must be derived from the authenticated Niv account and configured region."
  }
  ...
  assert {
    condition = (
      aws_s3_bucket.static_site.bucket == "sports-store-static-796231892654-eu-central-1" &&
...
run "additional_eks_principals_are_filtered_and_deduplicated" {
  ...
  variables {
    additional_eks_principal_arns = [
      "arn:aws:iam::796231892654:role/PlatformAdmin",
      "arn:aws:iam::796231892654:user/Niv-Bar",
      "arn:aws:iam::796231892654:role/PlatformAdmin",
    ]
  }
