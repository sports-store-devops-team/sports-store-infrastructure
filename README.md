...
This configuration targets AWS account `796231892654` in `eu-central-1`. The
default approved deployment and EKS administration principal is
`arn:aws:iam::796231892654:user/Niv-Bar`. Both Terraform roots and every
...
Niv's account currently has no GitHub Actions OIDC provider. The base Terraform
...
Niv's account must use two new, empty, separately locked states: one for
...
them only with Yuval's account for explicitly approved cleanup. Before Niv's
...
On a workstation authenticated to account `796231892654`, Niv reviews the expected plans and runs from the repository root:
...
1. Verifies account `796231892654` and validates both state roots.
2. Applies the base root in Niv's account.
...
After the first successful Terraform apply, Niv can inspect or populate the first version from a machine authenticated to the intended AWS account:
...
1. Verifies account `796231892654` and rejects unexpected base or CloudFront state before deletion.
...
5. Performs the scoped controller-security-group cleanup, destroys Niv's base state, and reports retained available EBS volumes. The base destroy permanently deletes all static files because `force_destroy = true`; scripts never delete the bucket or objects through AWS CLI.
