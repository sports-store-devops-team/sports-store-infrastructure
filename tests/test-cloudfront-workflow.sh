...
terraform() {
  ...
  case "$TERRAFORM_CALLS" in
    ...
    2)
      ...
      [[ "$7" == "-var=static_site_bucket_name=sports-store-static-796231892654-eu-central-1" ]] ||
        fail "S3 bucket name was not passed safely"
      [[ "$8" == "-var=static_site_bucket_arn=arn:aws:s3:::sports-store-static-796231892654-eu-central-1" ]] ||
        fail "S3 bucket ARN was not passed safely"
      [[ "$9" == "-var=static_site_bucket_regional_domain_name=sports-store-static-796231892654-eu-central-1.s3.eu-central-1.amazonaws.com" ]] ||
...
apply_cloudfront \
  "$VALID_HOSTNAME" \
  eu-central-1 \
  sports-store-static-796231892654-eu-central-1 \
  arn:aws:s3:::sports-store-static-796231892654-eu-central-1 \
  sports-store-static-796231892654-eu-central-1.s3.eu-central-1.amazonaws.com >/dev/null
